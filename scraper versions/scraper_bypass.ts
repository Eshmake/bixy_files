...
// NOTE: implemented everything minus stealth plugin
// This script contains a scraper w/ the new tech stack, and 
// logic that BYPASSES Captcha w/ 2Captcha service calls.
...
import RecaptchaPluginModule from "puppeteer-extra-plugin-recaptcha";
const RecaptchaPlugin = (RecaptchaPluginModule as any).default ?? RecaptchaPluginModule;

import type { Page, HTTPRequest, Dialog} from "puppeteer";
...
async function shutdown(browser?: any, exitCode = 0) {
  try {
  ...
  try {
    if (browser) {
      try {
        ...
        if (pages?.length) {
          await Promise.allSettled(pages.map((p: any) => p.close()));
        }
      } catch {}
...
async function safeGoto(page: Page, url: string, timeoutMs: number) {
  ...
  // --- ADDED: CAPTCHA SOLVING ---
  console.log('Detecting and solving reCAPTCHAs...');
  ...
  if (error) {
    ...
    console.log(`Successfully solved ${solved.length} CAPTCHAs.`);
  }
  // ------------------------------
...
async function vibrantPaletteFromFile(localPath: string, contentType?: string) {
  ...
  const needsConvert =
    ct.includes("image/webp") || ext.endsWith(".webp") ||
    ct.includes("image/avif") || ext.endsWith(".avif");
  ...
  // convert (webp/avif) -> png using sharp
  const sharpMod = await import("sharp");
  const sharp = (sharpMod as any).default ?? sharpMod;

  const pngPath = localPath.replace(/\.(webp|avif)$/i, ".png");
...
async function main() {
  ...
  try {
    ...
    page.on("dialog", (d: Dialog) => d.dismiss().catch(() => {}));
    ...
    // ---- CSS stats ----
    const styleStats = await page.evaluate((maxDomSample: number) => {
    ...
    const allImages = uniq(assets.images).filter((u): u is string => typeof u === "string").filter((u) => !looksLikeVideoUrl(u));
    ...
    await shutdown(browser, exitCode);
  }
}
